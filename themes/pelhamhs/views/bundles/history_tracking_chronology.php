<?php
/* ----------------------------------------------------------------------
 * bundles/ca_objects_history.php :    pelhamhs.org v1.2.2
 * Requires modified /app/lib/
 * ----------------------------------------------------------------------
 * CollectiveAccess
 ...
 */
	...
	$mode             = $policy_info["mode"];  //pelhamhs
	...
	$show_loan_controls = $show_movement_controls = $show_location_controls = $show_object_controls = $show_occurrence_controls = $show_collection_controls = $show_entity_controls = $show_movement_quickadd = false; //pelhamhs
?>
...
<?php
			...
			if(!$read_only && !caGetOption('hide_add_to_movement_controls', $settings, false) && ($subject_table::historyTrackingPolicyUses($policy, 'ca_movements'))) {
                if ($mode == "movements"){ //pelhamhs.org
                    $show_movement_quickadd = true;
?>                    
                    <div style='float: left;'  class='button caChangeLocationButton'><a href="#" id="<?php print $vs_id_prefix; ?>UpdateLocation"><?php print caNavIcon(__CA_NAV_ICON_ADD__, '15px'); ?> <?php print _t('Change Location'); ?></a></div>
<?php                
                }else{//$mode <> movements
...
?>		
		var panelContentID = '#' + caRelationQuickAddPanel<?php print $vs_id_prefix; ?>.getPanelContentID();
			...
			jQuery("#<?php print $vs_id_prefix; ?>UpdateLocation").on("click", function() { 
				caRelationQuickAddPanel<?php print $vs_id_prefix; ?>.showPanel('<?php print $vs_change_location_url; ?>'); 
				return false;
			});
<?php 
   }//if $show_movement_quickadd     pelhamhs
?>			
