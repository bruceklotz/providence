<?php
/** ---------------------------------------------------------------------
 * app/lib/Plugins/Media/PDFWand.php : PELHAMHS
 * ----------------------------------------------------------------------
 * CollectiveAccess
			...
			if (caMediaPluginGhostscriptInstalled($this->ops_ghostscript_path)) {
				...
				switch($ps_mimetype) { 
					case 'image/jpeg': //echo $this->ops_ghostscript_path." -dNOPAUSE -dBATCH -sDEVICE=".($vn_scaling_correction ? "tiff24nc" : "jpeg")." {$vs_antialiasing} -dJPEGQ=".$vn_quality." -dFirstPage=".$vn_start_page." -dLastPage=".$vn_end_page." -dMaxPatternBitmap=1000000 -dBandBufferSpace=500000000 -sBandListStorage=memory -dBufferSpace=1000000000 -dBandHeight=100 -sOutputFile=".caEscapeShellArg($ps_filepath.".".$vs_ext)." -r".$vs_res." -c \"30000000 setvmthreshold\" -f ".caEscapeShellArg($this->handle["filepath"]).(caIsPOSIX() ? " 2> /dev/null" : ""). $va_output. $vn_return;exit;
					  ...
					  if ($vn_scaling_correction){	system("convert " . caEscapeShellArg($ps_filepath.".".$vs_ext) . " " . caEscapeShellArg($ps_filepath.".".$vs_ext) );}//pelhamhs
