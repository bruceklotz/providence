<?php
/** ---------------------------------------------------------------------
 * app/lib/Plugins/InformationService/Lexicon4.php :   Pelhamhs
 * ----------------------------------------------------------------------
...
global $g_information_service_settings_Lexicon4;
$g_information_service_settings_Lexicon4 = array();/*
	'lang' => array(
		'formatType' => FT_TEXT,
		'displayType' => DT_FIELD,
		'default' => 'en',
		'width' => 30, 'height' => 1,
		'label' => _t('Lexicon4 language'),
		'description' => _t('2- or 3-letter language code for Lexicon4 to use. Defaults to "en". See http://meta.wikimedia.org/wiki/List_of_Lexicon4s')
	),
);*/

class WLPlugInformationServiceLexicon4 Extends BaseInformationServicePlugin Implements IWLPlugInformationService {
	# ------------------------------------------------
	static $s_settings;
	# ------------------------------------------------
	/**
	...
	public function __construct() {
		...
		WLPlugInformationServiceLexicon4::$s_settings = $g_information_service_settings_Lexicon4;
		parent::__construct();
		$this->info['NAME'] = 'Lexicon4';
		
		$this->description = _t('Provides access to Lexicon4 service');
	}
	# ------------------------------------------------
	/** 
	...
	public function getAvailableSettings() {
		return WLPlugInformationServiceLexicon4::$s_settings;
	}
	/** 
	 * Perform lookup on Lexicon4-based data service
	 *
	...
	public function lookup($pa_settings, $ps_search, $pa_options=null) {
		 $url = "http://pelhamhs.org/lexicon4.php?s={$ps_search}";
        ...
        $vs_result  = curl_exec($o_curl);
    		...
    		if($va_result = json_decode($vs_result, true)){
                 ...
                 $va_return['results'][] = array(
				           'label' => $va['Term']." [ $r_url ]",
				           'url' => htmlentities("http://pelhamhs.org/lexicon4.php?k=".$va['key']),
				           'idno' => $va['key']
		  	         );
		  	       }else{
		  	       	//error
	...
	# ------------------------------------------------
	/** 
	 * Fetch details about a specific item from a Lexicon4-based data service for "more info" panel
	 *
	 * @param array $pa_settings Plugin settings values
