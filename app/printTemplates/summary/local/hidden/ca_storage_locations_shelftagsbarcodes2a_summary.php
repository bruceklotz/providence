    ...
    function getListofObjectsInThisStorageLocation($this_location){
        ...
        if($this_contents = $this_location->getLocationContents('ca_movements'))
              ...
              foreach ( $objectsSorted as $objectindex => $item){
               ...
               $outx .=" <div class='itemrow' ><div class='div_box'><br/><br><span class='boxi' >&nbsp;&nbsp;&nbsp;&nbsp;</span></div>
                       <div class='div_idno'>$objectidno<br/>
                       <img src='$vs_path.png'/><br/>
                           <div class='casetype'>$type</div>&nbsp;
                       </div>";
                     
             //  if($showbarcodes !="no"){
                    //  $outx .= "<div class='div_barcode'><img src='$vs_path.png'/></div>";}//$showbarcodes
               //  }else{
                      //$outx .= "<div class='div_barcode'>	</div>";
             //    }//else	   
                
                 $outx .= "<div class='div_name'>$objectname &nbsp;</div>
                          <div class='div_image'>".$oneimage."&nbsp;</div>
                          <div class='div_location'>$storage_location<br/>
                 ...
                 $out .="<table class='tag_table'>
                    <tr class='tagrow'>	
                       <td class='col_box'><span class='box'>&nbsp;</span></td>
                       <td class='col_case'>$objectidno<br/>
                           <span class='casetype'>$type</span>&nbsp;
                       </td>";
                 if($showbarcodes !="no"){
                      $out .= "<td class='col_barcode'><span class='barcode'><img src='$vs_path.png'/><br/>$path</span>";}//$showbarcodes
                 else{
                      $out .= "<td class='col_barcode'><span class='barcode'><br/></span>";
                 }//else	   
                
                 $out .= "<span class='col_name'>$objectname &nbsp;</span></td>
                          <td class='col_image'>".$oneimage."&nbsp;</td>
                          <td class='col_location'>$storage_location
                              <span style='font-size:6px;'> $storage_location_idno <br/><br/>$dateactive</span>
                          </td>
                       </tr></table>";
                 ...
                 if($rowcount > $rowsperpage){ //check to see if we need a new page?
                     $rowcount =1;
                     //$itemcount =1;
                     $outx .= "<div class='pageBreak' style='page-break-before: always;'>&nbsp;</div><br/><br/>";
                 }
  ...
  if (!function_exists('getListofStorageInThisStorageLocation')){
    function getListofStorageInThisStorageLocation($this_location,$recursive){
        ...
        foreach ($va_storage_children as $va_key => $va_storage_children_id) {
            ...
            if ($vs_storage_type == "key") {$storagetypehtml = "<span class='circle'></span><span class='casetypeshelf'>$vs_storage_type</span>";}
            ...
            $t_storage_location = new ca_storage_locations($va_storage_children_id);
                ...
                <tr class='tagrow'>
                    <td class='col_box'><span class='box'>&nbsp;</span></td>
                    <td class='col_case'>$storage_locationsidno
                        $storagetypehtml
                    </td>";
             if($showbarcodes !="no"){
                $out .="<td style='display:rotate(90);' class='col_barcode'><span class='barcode'><img src='$vs_path.png'/><br/>$path</span>";}//$showbarcodes
             else{
             	  $out .="<td class='col_barcode'><span class='barcode'><br/></span>";
             }//else
             $out .=" <span class='col_name'>$storage_locationname</span></td>
                      <td class='col_image'>$oneimage&nbsp;</td>
                      <td class='col_location'>".print_r($this_storage,1)."
                        <span style='font-size:6px;'>".$x_statuss[$x_key]." ($recursive)</span>
                        <span style='font-size:6px;'>".print_r($this_storage,1)." <br/>$dateactive</span>
                      </td>
                </tr></table>";
...
// $out .= "<table class='tag_table'>";
 ...
 $css= "
     ...
     .storagepathdescription {width:100%;	text-align:center; font-size:12px;}
       ...
       .itemrow     {width:100%;height:$rowheight;border:1px solid black;background:yellow;max-height:$rowheight;}
      .div_box       {width:30px;height:$rowheight;float:left;margin-top:-10px}
      .boxi           {border:1px solid black; width:25px; height:25px; margin:5px; }
      .div_idno      {width:175px; padding-top:5px; font-size:18px; height:$rowheight;float:left;}
      ...
      .bartable      {rotate:90; width:$rowheight;height:$rowheight;border-top:2px dashed blue;}
       .bartable  tr    {width:$rowheight;height:$rowheight;}
       .pathtd      { width:$rowheight; height:$rowheight;font-size:6px; word-wrap:break-word; overflow-wrap:break-word;}
      .barcodei       {width:$rowheight; height:100px; }
     .barcodexi img   {height:$rowheight;}
     
     
      .div_name      {width:140px;  float:left; height:$halfheight;margin-top:25px}
      .div_image   { height:50px;float:left;width:100px; margin-top:-20px; border:1px solid black}     
      .div_image img    {max-height:50px;width:100px; }               
      .div_location  {width:100px; text-align:right;  font-size:14px; height:$rowheight;float:right;border:1px solid green;}
      .div_barcode       { height:45px; width:$rowheight;float:left;border:1px solid red;margin:15px;}
      .div_barcode img   { height:35px;margins:5px;;border:1px solid green;}                     
table.tag_table     {border-collapse:collapse;}
     ...
     .col_box       {width:50px; height:$rowheight}  
     ...
     .xbox           {border:1px solid black; width:25px; height:15px; margin:5px;}		
...
";//style sheet
   ...
   if(file_exists($this->request->getThemeDirectoryPath()."/graphics/logos/".$this->request->config->get('report_img'))){
	...
	//$vs_footer .= "<td class='footerText'  style='font-family: \"Sans Light\"; font-size: 12px; text-align: center;'><span id='version'>($version)</span></td>";
