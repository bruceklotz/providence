<?php
/* ----------------------------------------------------------------------
 * app/templates/footer.php : standard PDF report footer PELHAMHS
 * ----------------------------------------------------------------------
 * CollectiveAccess
 ...
 */
	...
	if($this->request->config->get('summary_header_enabled')) {
		...
		if($this->request->config->get('summary_show_timestamp')and !$this->getVar('hideTimestamp')) {
			$vs_footer .= "<td class='footerText'  style='font-family: \"Sans Light\"; font-size: 12px; text-align: right;'>".caGetLocalizedDate(null, array('dateFormat' => 'delimited'))."</td>";
		}
		$vs_footer .= "</tr></table>";
